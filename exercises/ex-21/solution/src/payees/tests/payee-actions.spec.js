...
describe( 'Testing payee-actions', () => {
  describe( 'getNextPrev', () => {
    test( 'Arguments should be passed to created action', () => {
      const payees = getPayees();
      const payee = payees[ 0 ];
      const direction = 'next';

      const action = getNextPrev( payee, payees, direction );

      expect( action.payee ).toBe( payee );
      expect( action.payees ).toBe( payees );
